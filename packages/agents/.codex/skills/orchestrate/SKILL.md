...
Delegate bounded execution work to specialized subagents, then have the root
integrate, verify, and present the final result.
...
explicitly asks for escalation or a Luna worker reports that the task requires
...
Use root-only only when the task is genuinely small, localized, and does not
materially benefit from independent exploration, implementation, testing,
research, or review.
...
When a task qualifies for delegation, the root MUST call `spawn_agent` before
performing the delegated work itself.
...
Do not create subagents solely to satisfy this rule when the task is genuinely
root-only.
...
> Trace where POST /invoices validates currency. Return the responsible files,
> validation path, and existing tests. Do not edit files.
...
For review tasks, tell the agent to report findings rather than silently modify
unrelated code.
...
When two or more delegated tasks are independent, spawn all of them before
...
Do not send multiple workers to edit the same files unless the root explicitly
...
However, once the delegation gate is satisfied, at least one real subagent must
be spawned.
...
Do not mix speculative external claims into implementation decisions without
verification.
...
Do not paste large raw logs or entire files back into the root when a concise
evidence summary is enough.
...
2. decide whether the task should be retried, narrowed, reassigned, or handled
   by the root
...
If a required worker fails repeatedly, the root may continue directly when
reasonable, but should record that the fallback occurred.
...
Do not claim delegation occurred unless `spawn_agent` was actually called
successfully.
...
Do not narrate every subagent action unless the user asks for detailed
